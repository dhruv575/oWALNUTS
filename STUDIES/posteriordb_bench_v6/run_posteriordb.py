#!/usr/bin/env python3
"""posteriordb benchmark v6 driver (the v5 protocol on the complete post-WP33 sampler defaults).
...
HARNESS = HERE / "target" / "release" / "posteriordb-bench-v6.exe"
FUNNEL = HERE / "target" / "release" / "funnel.exe"
ARMS = ["owalnuts-da", "cmdstan", "nutpie"]
OWALNUTS_MODE = {"owalnuts-da": "da"}
PRIOR_SUMMARY = HERE.parent / "posteriordb_bench_v5" / "artifacts" / "summary.json"
PRIOR_ARMS = {"owalnuts-da": "owalnuts-da", "cmdstan": "cmdstan", "nutpie": "nutpie"}
...
def metrics(model: str, arm: str, seed: int, draws: np.ndarray, names: list[str], ref: dict,
    ...
    return {
        "schema": "posteriordb-bench-v6-cell-metrics",
...
def failure(model: str, arm: str, seed: int, status: str, message: str, **extra) -> dict:
    return {
        "schema": "posteriordb-bench-v6-cell-metrics", "model": model, "arm": arm, "seed": seed,
...
def rescue_summary(chains: list[dict]) -> dict:
    """Aggregate the raw per-chain ChainRescueUpdate records."""
    restarted = 0
    by_criterion: dict[str, int] = {}
    events = []
    for chain in chains:
        for update in chain.get("chain_rescues", []):
            ...
            events.append({
                ...
                "source_position": outcome["source_position"],
                "step_before": update["step_before"],
                "step_after": outcome["step_after"],
                "median_log_density": update["median_log_density"],
                "log_density_iqr": update["log_density_iqr"],
            })
    return {
        "rescued_chains": restarted,
        "rescues_by_criterion": by_criterion,
...
def run_owalnuts(model: str, arm: str, seed: int, ref: dict) -> dict:
    ...
    extra = {
        ...
        "per_chain_bulk_ess_min": [
            float(np.nanmin(arviz_stats(draws[i:i + 1])["bulk_ess"]))
...
def analyze_funnel() -> dict:
    ...
    for c in cells:
        t = c["tail_mass"]
        rescue = rescue_summary(c["chains_data"])
...
def analyze() -> None:
    ...
    for model in models:
        ...
        for arm in ARMS:
            ...
            entry = {"n_cells": len(seeds), "n_ok": len(ok), "n_passed": sum(1 for c in ok if c["passed"]),
                     ...
                     "rescue_events": {
                         seed: c.get("rescue_events", [])
                         for seed, c in seeds.items() if c.get("status") == "ok"
                     },
    ...
    # v5 -> v6 comparison. The oWALNUTS difference is the chain-rescue default;
    # competitor changes are fresh-seed noise.
    prior = json.loads(PRIOR_SUMMARY.read_text(encoding="utf-8"))["per_model"] if PRIOR_SUMMARY.exists() else {}
    versus_v5 = {}
    for model in models:
        row = {}
        for arm, prior_arm in PRIOR_ARMS.items():
            current = per_model[model][arm]
            previous = prior.get(model, {}).get(prior_arm, {})

            def ratio(key, current=current, previous=previous):
                x = current[key]["median"]
                y = (previous.get(key) or {}).get("median")
                return (x / y) if (x and y) else None
            row[arm] = {
                "v5_passed": previous.get("n_passed"),
                "v6_passed": current["n_passed"],
                "v5_ok": previous.get("n_ok"),
                "v6_ok": current["n_ok"],
                "r_bulk_ess_per_gradient": ratio("min_bulk_ess_per_gradient"),
                "r_bulk_ess_per_second": ratio("min_bulk_ess_per_second"),
                "r_wall": ratio("wall_seconds"),
                "r_gradients": ratio("gradients_total"),
                "v5_bulk_ess_per_gradient": (previous.get("min_bulk_ess_per_gradient") or {}).get("median"),
                "v6_bulk_ess_per_gradient": current["min_bulk_ess_per_gradient"]["median"],
            }
        versus_v5[model] = row
    versus_v5_overall = {
        arm: {
            "models": [m for m in models if versus_v5[m][arm]["r_bulk_ess_per_gradient"]],
            "geomean_r_bulk_ess_per_gradient": geomean(
                [versus_v5[m][arm]["r_bulk_ess_per_gradient"] for m in models]
            ),
            "geomean_r_bulk_ess_per_second": geomean(
                [versus_v5[m][arm]["r_bulk_ess_per_second"] for m in models]
            ),
            "geomean_r_gradients": geomean(
                [versus_v5[m][arm]["r_gradients"] for m in models]
            ),
            "v5_cells_passed": sum((versus_v5[m][arm]["v5_passed"] or 0) for m in models),
            "v6_cells_passed": sum(versus_v5[m][arm]["v6_passed"] for m in models),
        }
    ...
    fixed = PROTOCOL["fixed_non_arma_models"]
    fixed_complete = [
        model for model in fixed
        if per_model[model]["owalnuts-da"]["n_ok"] == len(SEEDS)
        and per_model[model]["cmdstan"]["n_ok"] == len(SEEDS)
    ]
    fixed_ess_per_gradient = geomean([
        per_model[model]["owalnuts-da"]["min_bulk_ess_per_gradient"]["median"]
        / per_model[model]["cmdstan"]["min_bulk_ess_per_gradient"]["median"]
        for model in fixed_complete
    ])
    fixed_wall_per_gradient = geomean([
        per_model[model]["owalnuts-da"]["wall_per_gradient_us"]["median"]
        / per_model[model]["cmdstan"]["wall_per_gradient_us"]["median"]
        for model in fixed_complete
    ])
    regressed = {
        model: versus_v5[model]["owalnuts-da"]["r_bulk_ess_per_gradient"]
        for model in models
        if versus_v5[model]["owalnuts-da"]["r_bulk_ess_per_gradient"] is not None
        and versus_v5[model]["owalnuts-da"]["r_bulk_ess_per_gradient"] < 0.8
    }
    improved = {
        model: versus_v5[model]["owalnuts-da"]["r_bulk_ess_per_gradient"]
        for model in models
        if versus_v5[model]["owalnuts-da"]["r_bulk_ess_per_gradient"] is not None
        and versus_v5[model]["owalnuts-da"]["r_bulk_ess_per_gradient"] > 1.2
    }
    n_pass = sum(1 for c in da_cells if c.get("passed"))
    passing_z_violations = [
        f"{c['model']}/{c['seed']}={c['max_abs_z']:.3f}"
        for c in da_cells if c.get("passed") and c.get("max_abs_z", 0) > 4.0
    ]
    rescue_events = [
        {"model": model, "seed": int(seed), **event}
        for model in models
        for seed, c in cells.get(model, {}).get("owalnuts-da", {}).items()
        for event in c.get("rescue_events", [])
    ]
    ...
    recorded_cells = sum(per_model[m][arm]["n_cells"] for m in models for arm in ARMS)
    predictions = {
        "P1_da_gate_passes_ge_42_of_51": {
            "value": n_pass, "held": n_pass >= 42, "competitors": gates_by_arm
        },
        "P2_no_passing_da_cell_max_abs_z_gt_4": {
            "value": passing_z_violations, "held": not passing_z_violations
        },
        "P3_fixed_16_geomean_bulk_ess_per_gradient_vs_cmdstan_ge_0.75": {
            "value": fixed_ess_per_gradient, "models": len(fixed_complete),
            "held": (
                fixed_ess_per_gradient >= 0.75
                if len(fixed_complete) == len(fixed) else None
            ),
        },
        "P4_fixed_16_geomean_wall_per_gradient_vs_cmdstan_le_1.0": {
            "value": fixed_wall_per_gradient, "models": len(fixed_complete),
            "held": (
                fixed_wall_per_gradient <= 1.0
                if len(fixed_complete) == len(fixed) else None
            ),
        },
        "P5_da_geomean_bulk_ess_per_second_vs_nutpie_ge_1.5": {
            "value": hn["geomean_bulk_ess_per_second"], "models": len(hn["models"]),
            "held": bool(hn["geomean_bulk_ess_per_second"] >= 1.5),
        },
        "P6_funnel_defaults_abs_z_le_2_every_seed": {
            "value": fz,
            "held": all(abs(z) <= 2.0 for z in fz) if funnel_complete else None,
        },
        "P7_no_frozen_owalnuts_chain": {"value": da_frozen, "held": not da_frozen},
        "reported_all_153_cells_present": {"value": recorded_cells, "held": recorded_cells == 153},
        "reported_rescue_events": {"value": rescue_events},
        "reported_da_v6_over_v5_geomean_bulk_ess_per_gradient": {
            "value": versus_v5_overall["owalnuts-da"]["geomean_r_bulk_ess_per_gradient"]
        },
        "reported_da_models_below_0.8x_v5": {"value": regressed},
        "reported_da_models_above_1.2x_v5": {"value": improved},
        "reported_competitor_v6_over_v5_geomean_bulk_ess_per_gradient": {
            "value": {
                arm: versus_v5_overall[arm]["geomean_r_bulk_ess_per_gradient"]
                for arm in ("cmdstan", "nutpie")
            }
        },
    ...
    release_ready = bool(
        recorded_cells == 153
        and all(predictions[key]["held"] is True for key in (
            "P1_da_gate_passes_ge_42_of_51",
            "P2_no_passing_da_cell_max_abs_z_gt_4",
            "P3_fixed_16_geomean_bulk_ess_per_gradient_vs_cmdstan_ge_0.75",
            "P4_fixed_16_geomean_wall_per_gradient_vs_cmdstan_le_1.0",
    ...
    summary = {
        "schema": "posteriordb-bench-v6-summary",
        "versus_v5": versus_v5,
        "versus_v5_overall": versus_v5_overall,
        "fixed_non_arma_head_to_head": {
            "models_required": fixed,
            "models_complete": fixed_complete,
            "geomean_bulk_ess_per_gradient_vs_cmdstan": fixed_ess_per_gradient,
            "geomean_wall_per_gradient_vs_cmdstan": fixed_wall_per_gradient,
        },
    ...
    lines = ["# posteriordb benchmark v6 — results", "",
    ...
    lines += ["", "## v6 versus v5 (seed medians; ratio v6 / v5; seeds 90101-90103 versus 87101-87103; oWALNUTS adds the WP33 chain-rescue default, competitors show seed noise)", "",
              "| model | arm | v5 gates | v6 gates | v5 ESS/grad x1e3 | v6 ESS/grad x1e3 | ESS/grad v6/v5 | ESS/s v6/v5 | wall v6/v5 | grads v6/v5 |", "|---|---|---|---|---:|---:|---:|---:|---:|---:|"]
    for model in models:
        for arm in PRIOR_ARMS:
            v = versus_v5[model][arm]
            lines.append(f"| {model} | {arm} | {v['v5_passed']}/3 | {v['v6_passed']}/3 | {f((v['v5_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                         f"{f((v['v6_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | {f(v['r_bulk_ess_per_gradient'], '.2f')} | {f(v['r_bulk_ess_per_second'], '.2f')} | {f(v['r_wall'], '.2f')} | {f(v['r_gradients'], '.2f')} |")
    lines += ["", "| arm | v5 cells passed | v6 cells passed | geomean ESS/grad v6/v5 | geomean ESS/s v6/v5 | geomean grads v6/v5 |", "|---|---:|---:|---:|---:|---:|"]
    for arm, outcome in versus_v5_overall.items():
        lines.append(f"| {arm} | {outcome['v5_cells_passed']} | {outcome['v6_cells_passed']} | {f(outcome['geomean_r_bulk_ess_per_gradient'], '.2f')} | {f(outcome['geomean_r_bulk_ess_per_second'], '.2f')} | {f(outcome['geomean_r_gradients'], '.2f')} |")
    lines += ["", "## Chain-rescue events (owalnuts-da)", "",
              "| model | seed | chain | boundary transition | criterion | source | step before -> after | median log density |",
              "|---|---:|---:|---:|---|---:|---|---:|"]
    for event in rescue_events:
        lines.append(
            f"| {event['model']} | {event['seed']} | {event['chain']} | {event['transition']} | "
            f"{event['criterion']} | {event['source']} | {event['step_before']:.3g} -> {event['step_after']:.3g} | "
            f"{event['median_log_density']:.3g} |"
        )
    ...
    for seed, p in funnel["per_seed"].items():
        lines.append(f"| {seed} | {p['estimate']:.4f} | {p['z']:+.2f} | {p['z_batch_means']:+.2f} | {p['omega_bulk_ess']:.0f} / {p['omega_rhat']:.3f} | {p['target_calls_total']:,} | {p['divergences']} | {p['refinement_exhaustions']} | {p['rescued_chains']} | {', '.join(f'{x:.3g}' for x in p['final_step_size'])} |")
    if "pooled" in funnel:
        po = funnel["pooled"]
    ...
    lines += ["", f"**Release headline ready:** `{release_ready}`"]
