...
use owalnuts::walnutpie::{
    Error, InitialStepSearchConfig, KernelTuning, MultiChainOutput, PaperAdaptationConfig,
    ResearchTargetEvaluationLimit, RunConfig, RunControl, StopReason, StructuredBlockMass,
    StructuredCovarianceBlock, StructuredRefreshConfig, StructuredRefreshRestartPolicy, Target,
    LowRankArrowheadMass, ProjectedArrowheadWarmup, TargetError, WarmupConfig, WindowSummary,
    preflight_chains_projected_arrowhead, preflight_chains_structured,
    sample_chains_projected_arrowhead,
    preflight_chains_structured_refresh, sample_chains_structured,
...
fn summarize(
    ...
    format!(
        "{{\"schema\":\"flagship-crypto-sv-v2/run\",\"symbol\":\"{symbol}\",\"cell\":\"{cell}\",\"t\":{t},\"dim\":{},\
...
\"final_step_sizes\":{},\"final_max_errors\":{}}}",
        t + 3,
        output.chains().first().map_or(0, |c| c.diagnostics().len() - c.retained()),
        output.chains().first().map_or(0, |c| c.retained()),
...
impl Arm {
    ...
    fn uses_refresh(self) -> bool {
        matches!(self, Self::A | Self::B)
    }
}

fn run_config2(seed: u64, pilot: bool, arm: Arm) -> RunConfig {
    run_config2_with(seed, pilot, arm, None)
}

fn run_config2_with(seed: u64, pilot: bool, arm: Arm, retained_override: Option<usize>) -> RunConfig {
    let tuning = KernelTuning::new(
        INITIAL_STEP,
        NonZeroUsize::new(MAX_DEPTH).unwrap(),
        NonZeroUsize::new(MIN_MICRO).unwrap(),
    ...
    let warmup = match arm {
        Arm::O => WarmupConfig::new(TARGET_ACCEPTANCE)
            .unwrap()
            .with_mass_adaptation(false)
            .with_step_size_adaptation(true)
            .with_initial_step_search(InitialStepSearchConfig::default())
            .with_paper_adaptation(PaperAdaptationConfig::new(2.0, 0.95, 0.8).unwrap()),
        Arm::A | Arm::B | Arm::D => {
            WarmupConfig::new(if arm == Arm::B { 0.9 } else { 0.8 })
                .unwrap()
                .with_mass_adaptation(true)
                .with_step_size_adaptation(true)
                .with_initial_step_search(InitialStepSearchConfig::default())
        }
        Arm::C => WarmupConfig::new(TARGET_ACCEPTANCE)
            .unwrap()
            .with_mass_adaptation(false)
            .with_step_size_adaptation(true)
            .with_initial_step_search(InitialStepSearchConfig::default())
            .with_paper_adaptation(PaperAdaptationConfig::new(1.0, 0.95, 0.9).unwrap()),
        Arm::E => WarmupConfig::new(TARGET_ACCEPTANCE)
            .unwrap()
            .with_mass_adaptation(false)
            .with_step_size_adaptation(true)
            .with_initial_step_search(InitialStepSearchConfig::default())
            .with_paper_adaptation(PaperAdaptationConfig::new(1.0, 0.95, 0.95).unwrap()),
    };
    let retained = if pilot { 500 } else { retained_override.unwrap_or(RETAINED) };
    let discarded = if pilot { 300 } else { DISCARDED };
    RunConfig::new(discarded, NonZeroUsize::new(retained).unwrap(), seed)
        .with_tuning(tuning)
        .with_warmup(warmup)
...
/// Late-boundary rebuild: identity until the final slow-window boundary, then
/// install [tridiagonal global block from window precisions with stage-A
/// correlation structure; path block from window means] per PREREGISTRATION A1.
fn make_refresh(
    r: Vec<f64>,
    cal: &Calibration,
    discarded: usize,
) -> impl Fn(&WindowSummary, &StructuredBlockMass) -> Result<StructuredBlockMass, Error> + Send + Sync
{
    let stage = cal.global_precision_tridiagonal();
    let r01 = stage.off[0] / (stage.diag[0] * stage.diag[1]).sqrt();
    let r12 = stage.off[1] / (stage.diag[1] * stage.diag[2]).sqrt();
    move |summary: &WindowSummary, current: &StructuredBlockMass| {
        if summary.transition() + 60 < discarded {
            return Ok(current.clone());
        }
        let mean = summary.mean();
        let prec = summary.regularized_precision(&[0, 1, 2])?;
        let mut global = Tridiagonal {
            diag: prec.clone(),
            off: vec![
                r01 * (prec[0] * prec[1]).sqrt(),
                r12 * (prec[1] * prec[2]).sqrt(),
            ],
        };
        if !global.spd() {
            global = Tridiagonal {
        ...
        let window_cal = Calibration {
            mu_hat: mean[0],
            a_hat: mean[1],
            s_hat: mean[2],
            phi_hat: 2.0 * sigmoid(mean[1]) - 1.0,
            sigma_hat: mean[2].exp(),
            global_cov: [0.0; 9],
            h_mean: mean[3..].to_vec(),
        };
        let path = path_precision(&SvData { r: r.clone() }, &window_cal);
        let (gd, gs) = global.cholesky();
        let (pd, ps) = path.cholesky();
        StructuredBlockMass::new(vec![
...
/// SV target padded with three standard-normal dummy globals so the pooled
/// projected-arrowhead driver's `global == 6` requirement is met.
/// Layout: [d0, d1, d2, mu, a, s, h_1..h_T]; the dummies are independent
/// N(0,1) and do not change the SV posterior (Amendment A3 arm D).
struct PaddedSvTarget {
    inner: SvTarget,
}

impl Target for PaddedSvTarget {
    fn dimension(&self) -> usize {
        self.inner.dimension() + 3
    }
    ...
    fn log_density_gradient(
        ...
        let mut lp = 0.0;
        for i in 0..3 {
            lp += -0.5 * position[i] * position[i];
            gradient[i] = -position[i];
        }
        let inner_lp = self.inner.log_density_gradient(&position[3..], &mut gradient[3..])?;
        Ok(lp + inner_lp)
    }
}

fn write_draws_skip(path: &Path, output: &MultiChainOutput, skip: usize, dim: usize) {
    let mut file = fs::File::create(path).unwrap();
    for chain in output.chains() {
        let mut bytes = Vec::new();
        for draw in chain.samples().chunks(dim) {
            for value in &draw[skip..] {
                bytes.extend_from_slice(&value.to_le_bytes());
            }
        }
        file.write_all(&bytes).unwrap();
...
fn cmd_run_arrowhead(data_path: &Path, out: &Path, seed: u64, cal_path: &Path, label: &str, pilot: bool) {
    let (data, symbol, last, _) = SvData::load(data_path);
    let t = data.t();
    let sym = symbol.split('-').next().unwrap().to_string();
    fs::create_dir_all(out.join("draws")).unwrap();
    ...
    let cal = Calibration::load(cal_path);
    assert_eq!(cal.h_mean.len(), t, "calibration/data length mismatch");
    // 6x6 lower global factor: identity for the dummies, the tridiagonalized
    // stage-A global precision Cholesky for [mu, a, s].
    let (gl, gs) = cal.global_precision_tridiagonal().cholesky();
    let mut global_lower = vec![vec![0.0; 6]; 6];
    for (i, row) in global_lower.iter_mut().enumerate().take(3) {
        row[i] = 1.0;
    }
    for i in 0..3 {
        global_lower[3 + i][3 + i] = gl[i];
        if i > 0 {
            global_lower[3 + i][3 + i - 1] = gs[i - 1];
        }
    }
    let (pl, ps) = path_precision(&data, &cal).cholesky();
    let path_block = StructuredCovarianceBlock::BidiagonalCholesky { diagonal: pl, subdiagonal: ps };
    ...
    .unwrap();
    // Orthonormal level + centered-trend basis over the path coordinates.
    let level = 1.0 / (t as f64).sqrt();
    let mid = (t as f64 - 1.0) / 2.0;
    let trend_norm = (0..t).map(|i| (i as f64 - mid) * (i as f64 - mid)).sum::<f64>().sqrt();
    let basis: Vec<Vec<f64>> = (0..t)
        .map(|i| vec![level, (i as f64 - mid) / trend_norm])
        .collect();
    let spec = ProjectedArrowheadWarmup::new(
        basis,
    ...
    .unwrap();
    let base_starts = starts(&data, &cal, seed);
    let mut padded_starts = Vec::with_capacity(base_starts.len());
    for (chain, start) in base_starts.iter().enumerate() {
        let mut rng = SmallRng::seed_from_u64(seed.wrapping_add(7_777 + chain as u64));
        let n = Normal::new(0.0, 1.0).unwrap();
        let mut q = vec![n.sample(&mut rng), n.sample(&mut rng), n.sample(&mut rng)];
        q.extend_from_slice(start);
        padded_starts.push(q);
    }
    let target = PaddedSvTarget { inner: SvTarget { data, calls: AtomicUsize::new(0) } };
    let config = run_config2(seed, pilot, Arm::D);
    let report =
        preflight_chains_projected_arrowhead(&target, &padded_starts, &mass, &spec, &config)
            .unwrap();
    assert_eq!(target.inner.calls.load(Ordering::Relaxed), 0, "preflight started callbacks");
    let started = Instant::now();
    let output = sample_chains_projected_arrowhead(
        &target,
        &padded_starts,
        &mass,
        &spec,
    ...
    .unwrap_or_else(|e| panic!("{sym} seed {seed} {label} failed: {e:?}"));
    let wall = started.elapsed().as_secs_f64();
    let calls = target.inner.calls.load(Ordering::Relaxed);
    let stem = format!("{sym}-{label}-{seed}{}", if pilot { "-pilot" } else { "" });
    let summary = summarize(
        &sym,
        t,
        seed,
    ...
    let updates: Vec<String> = output
        ...
        .collect();
    fs::write(
        out.join("runs").join(format!("{stem}-metric.txt")),
        updates.join("
"),
    )
    .unwrap();
    write_draws_skip(&out.join("draws").join(format!("{stem}.f64")), output.chains(), 3, t + 6);
    eprintln!("{sym} T={t} last={last} seed={seed} {label}: {wall:.1}s, {calls} calls");
}
...
fn cmd_run2(
    ...
    assert_eq!(cal.h_mean.len(), t, "calibration/data length mismatch");
    let mass = build_mass(&data, &cal);
    let r_copy = data.r.clone();
    let target = SvTarget { data, calls: AtomicUsize::new(0) };
    let starts = starts(&target.data, &cal, seed);
    let config = run_config2_with(seed, pilot, arm, retained_override);
    let discarded = if pilot { 300 } else { DISCARDED };
    let stem = format!("{sym}-{label}-{seed}{}", if pilot { "-pilot" } else { "" });
    if arm.uses_refresh() {
        let refresh = make_refresh(r_copy, &cal, discarded);
        let report =
            preflight_chains_structured_refresh(&target, &starts, &mass, &config).unwrap();
        assert_eq!(target.calls.load(Ordering::Relaxed), 0, "preflight started callbacks");
        let started = Instant::now();
        let output = sample_chains_structured_refresh(
            &target,
            &starts,
            &mass,
            &refresh,
        ...
        fs::write(out.join("runs").join(format!("{stem}.json")), summary).unwrap();
        let mut rows = Vec::new();
        for (chain, updates) in output.metric_updates().iter().enumerate() {
            for u in updates {
                rows.push(format!(
                    "{{\"chain\":{chain},\"window\":{},\"transition\":{},\"outcome\":\"{:?}\",\
\"step_before\":{},\"step_after_search\":{},\"step_after_restart\":{}}}",
                    u.window_index(),
                    u.transition(),
                    u.outcome(),
                    fmt(u.step_before()),
                    u.step_after_search().map_or_else(|| "null".to_string(), fmt),
                    u.step_after_restart().map_or_else(|| "null".to_string(), fmt),
                ));
            }
        }
        fs::write(
            out.join("runs").join(format!("{stem}-refresh.json")),
            format!("[{}]", rows.join(",")),
        )
        .unwrap();
        write_draws(&out.join("draws").join(format!("{stem}.f64")), output.chains());
        eprintln!("{sym} T={t} last={last} seed={seed} {label}: {wall:.1}s, {calls} calls");
    } else {
        let report = preflight_chains_structured(&target, &starts, &mass, &config).unwrap();
        assert_eq!(target.calls.load(Ordering::Relaxed), 0, "preflight started callbacks");
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run2") if args.get(6).map(String::as_str) == Some("D") => {
            let pilot = args.get(8).map(String::as_str) == Some("pilot");
            cmd_run_arrowhead(
                Path::new(&args[2]),
                &PathBuf::from(&args[3]),
                args[4].parse().unwrap(),
        ...
        Some("run2") => {
            ...
            let retained_override = args
                .iter()
                .find_map(|a| a.strip_prefix("retained=").and_then(|v| v.parse().ok()));
            cmd_run2(
                Path::new(&args[2]),
                &PathBuf::from(&args[3]),
                args[4].parse().unwrap(),
