...
INITIAL_STEP = 0.1
TARGET_ACCEPT = 0.8
# v2 arm configurations (PREREGISTRATION.md A1/A3): set by argv[4].
ARM = "C"
LATE_REBUILD_MARGIN = 60
...
def make_refresh(r, cal, discarded):
    """Numpy mirror of the native make_refresh (PREREGISTRATION A1)."""
    import numpy as np

    cov = GLOBAL_COV_INFLATION * np.asarray(cal["global_cov"]).reshape(3, 3)
    prec0 = np.linalg.inv(cov)
    d0 = np.sqrt(np.diag(prec0))
    r01 = prec0[0, 1] / (d0[0] * d0[1])
    r12 = prec0[1, 2] / (d0[1] * d0[2])

    def refresh(window, transition, count, mean, variance):
        if transition + LATE_REBUILD_MARGIN < discarded:
            return None
        shrink = count / (count + 5.0)
        prec = 1.0 / (shrink * variance[:3] + (1.0 - shrink) * 1.0)
        gdiag = prec
        goff = np.array([r01 * np.sqrt(prec[0] * prec[1]), r12 * np.sqrt(prec[1] * prec[2])])
        try:
            import owalnuts
            gblock = owalnuts.tridiagonal_precision_mass(gdiag, goff)
        except Exception:
            import owalnuts
            gblock = owalnuts.tridiagonal_precision_mass(gdiag, np.zeros(2))
        phi = 2.0 / (1.0 + np.exp(-mean[1])) - 1.0
        s2 = np.exp(2.0 * mean[2])
        h_mean = np.asarray(mean[3:])
        t_len = len(r)
        diag = np.full(t_len, (1.0 + phi * phi) / s2)
        diag[0] = diag[-1] = 1.0 / s2
        diag += 0.5 * r * r * np.exp(-h_mean)
        off = np.full(t_len - 1, -phi / s2)
        import owalnuts
        return gblock + owalnuts.tridiagonal_precision_mass(diag, off)
...
def cell_pymc(symbol: str, seed: int, pilot: bool):
        ...
        adaptation=(
            owalnuts.Adaptation(
                ...
                paper=owalnuts.PaperAdaptation(
                    global_energy_bound=1.0,
                    unrefined_fraction_target=(0.95 if ARM == "E" else 0.9),
                ),
            )
            if ARM in ("C", "E")
            else owalnuts.Adaptation(target_accept=0.9, adapt_mass=True)
        ),
        mass=mass,
        refresh=(None if ARM in ("C", "E") else make_refresh(r, cal, warmup)),
        max_target_evaluations=RESEARCH_LIMIT,
    )
    wall = time.perf_counter() - began
    meta = {
        "schema": "flagship-crypto-sv-v2/run",
        "symbol": symbol,
        "cell": _CELL_LABEL,
        "seed": seed,
        "pilot": pilot,
    ...
    save_outputs(symbol, _CELL_LABEL, seed, result.samples, meta)


def cell_pymc_b(symbol: str, seed: int, pilot: bool):
    """Fallback arm B (A7): standard DA 0.9 + late rebuild, cell name pymcB."""
    global ARM
    ARM = "B"
    _orig = save_outputs.__defaults__
    # Reuse cell_pymc but relabel outputs by monkey-patching the save call.
    import types
    global _CELL_LABEL
    _CELL_LABEL = "pymcB"
    cell_pymc(symbol, seed, pilot)
    _CELL_LABEL = "pymc"
