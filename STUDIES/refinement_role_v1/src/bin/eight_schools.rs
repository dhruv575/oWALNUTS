...
//! timing boundary copied verbatim, with the sampler's current defaults —
//! `MomentumSum`, Stan regularisation — as in `joint_default_v1`'s `joint`
//! arm) under one of the arms of the refinement-role study (`arms.rs`): the
//! arm's step rule replaces the strict track's `WarmupConfig::new(0.95)`
//! where it changes it (`Arm::facade_warmup`), and the arm's `delta`
//! replaces the strict track's `delta = 1` in `KernelTuning`.
//!
//! Usage: `eight-schools <arm> <seed> <reps> <out.json>`
//!
...
const SEEDS: [u64; 3] = [89_101, 89_102, 89_103];
const STRICT_TARGET: f64 = 0.95;
...
fn config(seed: u64, arm: &Arm) -> Result<RunConfig, Box<dyn Error>> {
    Ok(RunConfig::new(1000, NonZeroUsize::new(1000).unwrap(), seed)
        ...
        .with_warmup(arm.facade_warmup(STRICT_TARGET)?))
}
    ...
    let chains = sampled
        ...
        .map(|chain| {
            ...
            json!({
                ...
                "retained_refinement_level_built": retained.refinement_level_built(),
                "retained_reverse_coarser_stops": retained.reverse_coarser_stops(),
            })
        })
        .collect::<Vec<_>>();
    let payload = json!({
        "schema": "refinement-role-v1-eight-schools-cell",
        "arm": arm.name,
        "warmup_config": arm.json(),
        "strict_target_accept": STRICT_TARGET,
        "seed": seed,
        "starts": starts(),
        "warmup": 1000, "retained": 1000, "chains": CHAINS,
        "depth": 8, "metric": "adapted diagonal", "threads": 1,
        "tuning": {"step_size": 0.3, "max_depth": 8, "min_micro_steps": 1,
                   "max_refinement_levels": 8, "max_error": arm.max_error, "divergence_threshold": 1000.0},
        "callback_cap": CAP,
...
fn main() {
    ...
    let result: Result<(), Box<dyn Error>> = match args.as_slice() {
        ...
        _ => Err("usage: <arm> <seed> <reps> <out.json>".into()),
    };
