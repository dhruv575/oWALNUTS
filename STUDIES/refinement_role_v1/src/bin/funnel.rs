...
//! refinement-role study at the sampler defaults.
//!
//! Usage: `funnel defaults <arm> <seed> <out.json>`
//!
//! `defaults`: `Tuning::default()` (eight levels, `MomentumSum`), adapted
//! diagonal metric with the Stan regularisation, plus the arm's step rule
//! and `delta` (`arms.rs`). Four starts `omega in {-3, -1, 1, 3}`, 2,000 warmup and 20,000
...
fn run(tuning_name: &str, arm_name: &str, seed: u64, out: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let retained_calls: usize = posterior
        ...
        .sum();
    let payload = json!({
        "schema": "refinement-role-v1-funnel",
