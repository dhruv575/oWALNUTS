...
CELLS = ["native", "pymc", "nutpie", "numpyro"]
SEEDS = {"native": [97001, 97002, 97003], "pymc": [97001, 97002, 97003],
...
def load_functionals(symbol, cell, seed):
    if cell == "native":
        dim = T[symbol] + 3
        path = ROOT / "artifacts" / "draws" / f"{symbol}-native-{seed}.f64"
    ...
    d = np.load(ROOT / "artifacts" / "draws" / f"{symbol}-{cell}-{seed}.npz")
    return {k: d[k] for k in ("mu", "a", "s", "phi", "sigma", "h_T", "mean_h")}


def load_meta(symbol, cell, seed):
    meta = json.loads((ROOT / "artifacts" / "runs" / f"{symbol}-{cell}-{seed}.json").read_text())
...
def main():
    ...
    summary = {"schema": "flagship-crypto-sv-v1/summary", "results": results, "agreement": agreement}
