#!/usr/bin/env python3
"""posteriordb benchmark v2 driver (rerun of v1 on the sampler API with fresh seeds).
...
HARNESS = HERE / "target" / "release" / "posteriordb-bench-v2.exe"
ARMS = ["owalnuts-da", "owalnuts-paper", "owalnuts-stan-style", "cmdstan", "nutpie"]
OWALNUTS_MODE = {"owalnuts-da": "da", "owalnuts-paper": "paper", "owalnuts-stan-style": "stan"}
V1_SUMMARY = HERE.parent / "posteriordb_bench_v1" / "artifacts" / "summary.json"
V1_ARMS = {"owalnuts-da": "owalnuts-da", "owalnuts-paper": "owalnuts-paper", "cmdstan": "cmdstan", "nutpie": "nutpie"}
...
def metrics(model: str, arm: str, seed: int, draws: np.ndarray, names: list[str], ref: dict,
    ...
    return {
        "schema": "posteriordb-bench-v2-cell-metrics",
...
def failure(model: str, arm: str, seed: int, status: str, message: str, **extra) -> dict:
    return {
        "schema": "posteriordb-bench-v2-cell-metrics", "model": model, "arm": arm, "seed": seed,
...
def analyze() -> None:
    ...
    for arm in ("cmdstan", "nutpie"):
        for ours in ("owalnuts-da", "owalnuts-paper", "owalnuts-stan-style"):
    ...
    # v1 -> v2 comparison (seed medians; v1 arms at their v1 settings).
    v1 = json.loads(V1_SUMMARY.read_text(encoding="utf-8"))["per_model"] if V1_SUMMARY.exists() else {}
    versus_v1 = {}
    for model in models:
        row = {}
        for arm, v1arm in V1_ARMS.items():
            a, b = per_model[model][arm], v1.get(model, {}).get(v1arm, {})

            def r(k, a=a, b=b):
                x = a[k]["median"]
                y = (b.get(k) or {}).get("median")
                return (x / y) if (x and y) else None
            row[arm] = {"v1_passed": b.get("n_passed"), "v2_passed": a["n_passed"], "v1_ok": b.get("n_ok"), "v2_ok": a["n_ok"],
                        ...
                        "v1_bulk_ess_per_gradient": (b.get("min_bulk_ess_per_gradient") or {}).get("median"),
                        "v2_bulk_ess_per_gradient": a["min_bulk_ess_per_gradient"]["median"]}
        versus_v1[model] = row
    versus_v1_overall = {arm: {
        "models": [m for m in models if versus_v1[m][arm]["r_bulk_ess_per_gradient"]],
        "geomean_r_bulk_ess_per_gradient": geomean([versus_v1[m][arm]["r_bulk_ess_per_gradient"] for m in models]),
        "geomean_r_bulk_ess_per_second": geomean([versus_v1[m][arm]["r_bulk_ess_per_second"] for m in models]),
        "v1_cells_passed": sum((versus_v1[m][arm]["v1_passed"] or 0) for m in models),
        "v2_cells_passed": sum(versus_v1[m][arm]["v2_passed"] for m in models),
    } for arm in V1_ARMS}
    ...
    paper_frozen = [f"{m}/{s}" for m in models for s, c in cells.get(m, {}).get("owalnuts-paper", {}).items() if c.get("frozen_chains", 0) > 0]
    hc = head["owalnuts-da_over_cmdstan"]
    predictions = {
        "P1_da_gate_passes_ge_33_of_51": {"value": sum(1 for c in da_cells if c.get("passed")), "held": sum(1 for c in da_cells if c.get("passed")) >= 33},
        "P2_da_geomean_bulk_ess_per_gradient_vs_cmdstan_ge_0.45": {"value": hc["geomean_bulk_ess_per_gradient"], "held": bool(hc["geomean_bulk_ess_per_gradient"] >= 0.45)},
        "P3_da_wall_per_gradient_within_1.5x_cmdstan": {"value": hc["geomean_wall_per_gradient"], "held": bool(hc["geomean_wall_per_gradient"] <= 1.5)},
        "P4_paper_arm_no_frozen_cells": {"value": paper_frozen, "held": not paper_frozen},
        "P5_zero_owalnuts_cells_lost_to_fatal_nan_or_start_failure": {"value": fatal, "held": not fatal},
    }
    summary = {
        "schema": "posteriordb-bench-v2-summary",
        "versus_v1": versus_v1,
        "versus_v1_overall": versus_v1_overall,
    ...
    lines = ["# posteriordb benchmark v2 — results", "",
    ...
    lines += ["", "## v2 versus v1 (seed medians; ratio v2 / v1; v1 arms at their v1 settings: depth 8, h0 0.1, STAN_THREADS build)", "",
              "| model | arm | v1 gates | v2 gates | v1 ESS/grad x1e3 | v2 ESS/grad x1e3 | ESS/grad v2/v1 | ESS/s v2/v1 | wall v2/v1 | grads v2/v1 |", "|---|---|---|---|---:|---:|---:|---:|---:|---:|"]
    for model in models:
        for arm in V1_ARMS:
            v = versus_v1[model][arm]
            lines.append(f"| {model} | {arm} | {v['v1_passed']}/3 | {v['v2_passed']}/3 | {f((v['v1_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                         f"{f((v['v2_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | {f(v['r_bulk_ess_per_gradient'], '.2f')} | {f(v['r_bulk_ess_per_second'], '.2f')} | {f(v['r_wall'], '.2f')} | {f(v['r_gradients'], '.2f')} |")
    lines += ["", "| arm | v1 cells passed | v2 cells passed | geomean ESS/grad v2/v1 | geomean ESS/s v2/v1 |", "|---|---:|---:|---:|---:|"]
    for arm, o in versus_v1_overall.items():
        lines.append(f"| {arm} | {o['v1_cells_passed']} | {o['v2_cells_passed']} | {f(o['geomean_r_bulk_ess_per_gradient'], '.2f')} | {f(o['geomean_r_bulk_ess_per_second'], '.2f')} |")
    lines += ["", "## Preregistered predictions", "", "| prediction | value | held |", "|---|---|---|"]
    for k, v in predictions.items():
        val = v["value"]
        lines.append(f"| {k} | {f(val, '.3f') if isinstance(val, float) else val} | {v['held']} |")
