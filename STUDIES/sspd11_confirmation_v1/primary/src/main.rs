//! WP12: fresh-seed confirmation of WP4b arms I (centered adapted diagonal)
//! and P (fixed posterior-precision path block) on sspd-11, kernel v10.
//! Runner copied from `STUDIES/real_target_path_metric_v1/src/main.rs`;
//! the only change is that `protocol.seeds[fixture]` is a list and every
//! arm runs once per seed (P uses arm I's adapted globals mass of the same seed).
...
fn main() {
    ...
    let mut arms = "I,P".to_string();
    let mut out = PathBuf::from("artifacts/primary-v1");
    ...
    for key in fixtures.split(',') {
        ...
        .unwrap();
        let seeds: Vec<u64> = protocol["seeds"][key].as_array().unwrap().iter().map(|v| v.as_u64().unwrap()).collect();
    ...
    fs::write(
        out.join("index.json"),
        serde_json::to_string_pretty(&json!({"schema": "sspd11-confirmation-v1/runs", "algorithm_revision": ALGORITHM_REVISION, "kernel_commit": kernel_commit, "runs": index})).unwrap(),
    )
    .unwrap();
}
