#!/usr/bin/env python3
"""posteriordb benchmark v4 driver (the v3 protocol on the WP28 eight-level default plus a Stan-regularisation arm, fresh seeds).
...
HARNESS = HERE / "target" / "release" / "posteriordb-bench-v4.exe"
ARMS = ["owalnuts-da", "owalnuts-da-stanreg", "cmdstan", "nutpie"]
OWALNUTS_MODE = {"owalnuts-da": "da", "owalnuts-da-stanreg": "stanreg"}
V3_SUMMARY = HERE.parent / "posteriordb_bench_v3" / "artifacts" / "summary.json"
# v4 arm -> v3 arm it is compared with (the stanreg arm is compared with the v3 DA arm: it is the DA warmup plus one option).
V3_ARMS = {"owalnuts-da": "owalnuts-da", "owalnuts-da-stanreg": "owalnuts-da", "cmdstan": "cmdstan", "nutpie": "nutpie"}
...
# (the v3 libraries no longer exist; same BridgeStan 2.9.0 / Stan 2.39.0 / flags).
...
def metrics(model: str, arm: str, seed: int, draws: np.ndarray, names: list[str], ref: dict,
    ...
    return {
        "schema": "posteriordb-bench-v4-cell-metrics",
...
def failure(model: str, arm: str, seed: int, status: str, message: str, **extra) -> dict:
    return {
        "schema": "posteriordb-bench-v4-cell-metrics", "model": model, "arm": arm, "seed": seed,
...
def run_owalnuts(model: str, arm: str, seed: int, ref: dict) -> dict:
    ...
    hist = extra["retained_refinement_level_histogram"]
    extra["refined_fraction"] = (sum(hist[1:]) / sum(hist)) if sum(hist) else None
    extra["refined_fraction_per_chain"] = [(sum(c["retained_refinement_level_histogram"][1:]) / sum(c["retained_refinement_level_histogram"]))
                                           if sum(c["retained_refinement_level_histogram"]) else None for c in chains]
    extra["refined_beyond_level_4_fraction"] = (sum(hist[5:]) / sum(hist)) if sum(hist) else None
...
def analyze() -> None:
    ...
    keys = ["min_bulk_ess_per_second", "min_tail_ess_per_second", "min_bulk_ess_per_second_sampling",
            ...
            "wall_per_gradient_us", "refined_fraction", "refined_beyond_level_4_fraction", "max_depth_stops"]
    per_model = {}
    for model in models:
        row = {}
        for arm in ARMS:
            ...
            entry = {"n_cells": len(seeds), "n_ok": len(ok), "n_passed": sum(1 for c in ok if c["passed"]),
                     ...
                     "max_refinement_levels": sorted({c.get("max_refinement_levels") for c in ok if c.get("max_refinement_levels") is not None}),
                     "refinement_exhaustions": [c.get("refinement_exhaustions") for c in ok],
    ...
    for arm in ("cmdstan", "nutpie"):
        for ours in ("owalnuts-da", "owalnuts-da-stanreg"):
    ...
    # stanreg versus DA within the study (seed medians; the same seeds and starts; per-model ratio and gates).
    stanreg_vs_da = {}
    for model in models:
        a, b = per_model[model]["owalnuts-da-stanreg"], per_model[model]["owalnuts-da"]

        def r(k, a=a, b=b):
            x, y = a[k]["median"], b[k]["median"]
            return (x / y) if (x and y) else None
        stanreg_vs_da[model] = {"da_passed": b["n_passed"], "stanreg_passed": a["n_passed"], "da_ok": b["n_ok"], "stanreg_ok": a["n_ok"],
                                ...
                                "da_bulk_ess_per_gradient": b["min_bulk_ess_per_gradient"]["median"],
                                "stanreg_bulk_ess_per_gradient": a["min_bulk_ess_per_gradient"]["median"],
                                "da_step": b["final_step_size"], "stanreg_step": a["final_step_size"],
                                "gate_lost": a["n_passed"] < b["n_passed"]}
    stanreg_ratios = {m: stanreg_vs_da[m]["r_bulk_ess_per_gradient"] for m in models if stanreg_vs_da[m]["r_bulk_ess_per_gradient"]}
    stanreg_overall = {
        "models": sorted(stanreg_ratios),
        "geomean_r_bulk_ess_per_gradient": geomean(list(stanreg_ratios.values())),
        "geomean_r_bulk_ess_per_second": geomean([stanreg_vs_da[m]["r_bulk_ess_per_second"] for m in models]),
        "min_model": min(stanreg_ratios, key=stanreg_ratios.get) if stanreg_ratios else None,
        "min_ratio": min(stanreg_ratios.values()) if stanreg_ratios else None,
        "models_below_0.8": {m: v for m, v in stanreg_ratios.items() if v < 0.8},
        "gates_lost": [m for m in models if stanreg_vs_da[m]["gate_lost"]],
        "da_cells_passed": sum(stanreg_vs_da[m]["da_passed"] for m in models),
        "stanreg_cells_passed": sum(stanreg_vs_da[m]["stanreg_passed"] for m in models),
    }
    # v3 -> v4 comparison (seed medians; identical protocol; the v3 four-level default versus the v4 eight-level default,
    # plus the stanreg arm against the v3 DA arm).
    ...
    for model in models:
        ...
        for arm, v3arm in V3_ARMS.items():
            ...
            def r(k, a=a, b=b):
                ...
                return (x / y) if (x and y) else None
            row[arm] = {"v3_arm": v3arm, "v3_passed": b.get("n_passed"), "v4_passed": a["n_passed"], "v3_ok": b.get("n_ok"), "v4_ok": a["n_ok"],
                        "r_bulk_ess_per_gradient": r("min_bulk_ess_per_gradient"), "r_bulk_ess_per_second": r("min_bulk_ess_per_second"),
                        "r_wall": r("wall_seconds"), "r_gradients": r("gradients_total"),
                        "v3_bulk_ess_per_gradient": (b.get("min_bulk_ess_per_gradient") or {}).get("median"),
                        "v4_bulk_ess_per_gradient": a["min_bulk_ess_per_gradient"]["median"]}
        versus_v3[model] = row
    versus_v3_overall = {arm: {
        ...
        "v4_cells_passed": sum(versus_v3[m][arm]["v4_passed"] for m in models),
    } for arm in V3_ARMS}
    refined = {m: {"da": per_model[m]["owalnuts-da"]["refined_fraction"]["median"],
                   "da_beyond_level_4": per_model[m]["owalnuts-da"]["refined_beyond_level_4_fraction"]["median"],
                   "stanreg": per_model[m]["owalnuts-da-stanreg"]["refined_fraction"]["median"]} for m in models}
    ...
    v3_da_vs_cmdstan = 0.344
    n_pass = sum(1 for c in da_cells if c.get("passed"))
    sblrc, earn = "sblrc-blr", "earnings-logearn_interaction"
    levels = sorted({lv for m in models for arm in OWALNUTS_MODE for lv in per_model[m][arm]["max_refinement_levels"]})
    predictions = {
        "P1_da_gate_passes_ge_35_of_51": {"value": n_pass, "held": n_pass >= 35},
        "P2_da_geomean_bulk_ess_per_gradient_vs_cmdstan_within_0.9_1.1x_of_v3_0.344": {
            "value": hc["geomean_bulk_ess_per_gradient"], "models": len(hc["models"]),
            "ratio_to_v3": hc["geomean_bulk_ess_per_gradient"] / v3_da_vs_cmdstan,
            "held": bool(0.9 * v3_da_vs_cmdstan <= hc["geomean_bulk_ess_per_gradient"] <= 1.1 * v3_da_vs_cmdstan)},
        "P3_stanreg_over_da_bulk_ess_per_gradient_ge_2x_on_sblrc_and_earnings": {
            "value": {"sblrc": stanreg_vs_da[sblrc]["r_bulk_ess_per_gradient"], "earnings": stanreg_vs_da[earn]["r_bulk_ess_per_gradient"]},
            "held": bool((stanreg_vs_da[sblrc]["r_bulk_ess_per_gradient"] or 0) >= 2 and (stanreg_vs_da[earn]["r_bulk_ess_per_gradient"] or 0) >= 2)},
        "P4_stanreg_over_da_geomean_ge_1.1_and_no_model_below_0.8": {
            "value": {"geomean": stanreg_overall["geomean_r_bulk_ess_per_gradient"], "min_model": stanreg_overall["min_model"],
                      "min_ratio": stanreg_overall["min_ratio"], "models_below_0.8": stanreg_overall["models_below_0.8"]},
            "held": bool(stanreg_overall["geomean_r_bulk_ess_per_gradient"] >= 1.1 and not stanreg_overall["models_below_0.8"])},
        "P5_da_wall_per_gradient_le_1.0x_cmdstan": {"value": hc["geomean_wall_per_gradient"], "held": bool(hc["geomean_wall_per_gradient"] <= 1.0)},
        "default_rule_stanreg_ge_1.1_geomean_no_model_below_0.8_no_gate_lost": {
            "value": {"geomean": stanreg_overall["geomean_r_bulk_ess_per_gradient"], "models_below_0.8": stanreg_overall["models_below_0.8"],
                      "gates_lost": stanreg_overall["gates_lost"], "da_cells": stanreg_overall["da_cells_passed"], "stanreg_cells": stanreg_overall["stanreg_cells_passed"]},
            "held": bool(stanreg_overall["geomean_r_bulk_ess_per_gradient"] >= 1.1 and not stanreg_overall["models_below_0.8"] and not stanreg_overall["gates_lost"])},
        "reported_max_refinement_levels_in_cells": {"value": levels},
        "reported_da_refined_fraction_per_model": {"value": refined},
        "reported_da_frozen_cells": {"value": da_frozen},
        "reported_owalnuts_cells_not_ok": {"value": fatal},
    }
    summary = {
        "schema": "posteriordb-bench-v4-summary",
        "stanreg_vs_da": stanreg_vs_da,
        "stanreg_vs_da_overall": stanreg_overall,
        "versus_v3": versus_v3,
    ...
    lines = ["# posteriordb benchmark v4 — results", "",
    ...
    for model in models:
        for arm in ARMS:
            ...
            lines.append(
                ...
                f"{f(e['refined_fraction']['median'], '.3f')} | {f(e['max_depth_stops']['median'], ',.0f')} |")
    lines += ["",
    ...
    def hs(steps):
        return "—" if not steps else "/".join(f"{h:.2g}" for h in steps[0])

    lines += ["", "## owalnuts-da-stanreg versus owalnuts-da (same seeds and starts; seed medians; ratio stanreg / da; final h per chain on the first seed)", "",
    ...
    for model in models:
        v = stanreg_vs_da[model]
        lines.append(f"| {model} | {v['da_passed']}/3 | {v['stanreg_passed']}/3 | {f((v['da_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                     f"{f((v['stanreg_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | {f(v['r_bulk_ess_per_gradient'], '.2f')} | {f(v['r_bulk_ess_per_second'], '.2f')} | "
                     f"{f(v['r_gradients'], '.2f')} | {hs(v['da_step'])} | {hs(v['stanreg_step'])} |")
    o = stanreg_overall
    lines += ["", f"geomean ESS/grad ratio {f(o['geomean_r_bulk_ess_per_gradient'], '.3f')} over {len(o['models'])} models; min {o['min_model']} {f(o['min_ratio'], '.2f')}; "
              f"models below 0.8x: {o['models_below_0.8'] or 'none'}; gates lost (stanreg passes fewer seeds than da): {o['gates_lost'] or 'none'}; "
              f"cells passing da {o['da_cells_passed']} / stanreg {o['stanreg_cells_passed']}."]
    lines += ["", "## v4 versus v3 (seed medians; ratio v4 / v3; same protocol, seeds 79101-79103 -> 83101-83103; the sampler change is `max_refinement_levels` 4 -> 8 in every oWALNUTS arm; the stanreg arm is compared with the v3 DA arm)", "",
    ...
    for model in models:
        for arm in V3_ARMS:
            ...
            lines.append(f"| {model} | {arm} (v3 {v['v3_arm']}) | {v['v3_passed']}/3 | {v['v4_passed']}/3 | {f((v['v3_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                         f"{f((v['v4_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | {f(v['r_bulk_ess_per_gradient'], '.2f')} | {f(v['r_bulk_ess_per_second'], '.2f')} | {f(v['r_wall'], '.2f')} | {f(v['r_gradients'], '.2f')} |")
    lines += ["", "| arm | v3 cells passed | v4 cells passed | geomean ESS/grad v4/v3 | geomean ESS/s v4/v3 |", "|---|---:|---:|---:|---:|"]
    for arm, o in versus_v3_overall.items():
        lines.append(f"| {arm} (v3 {o['v3_arm']}) | {o['v3_cells_passed']} | {o['v4_cells_passed']} | {f(o['geomean_r_bulk_ess_per_gradient'], '.2f')} | {f(o['geomean_r_bulk_ess_per_second'], '.2f')} |")
    lines += ["", "## Retained transitions that refined (fraction with selected refinement level > 0; seed medians over the four chains' 4,000 retained transitions)", "",
              "| model | owalnuts-da | da at level 5-8 | owalnuts-da-stanreg |", "|---|---:|---:|---:|"]
    for model in models:
        r_ = refined[model]
        lines.append(f"| {model} | {f(r_['da'], '.4f')} | {f(r_['da_beyond_level_4'], '.5f')} | {f(r_['stanreg'], '.4f')} |")
