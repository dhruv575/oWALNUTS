//! oWALNUTS arms of the posteriordb benchmark v4: one (model, arm, seed) cell.
//!
//! Identical to the v3 harness except that it is built against the current
//! `src/`, whose `Tuning::default()` now has `max_refinement_levels = 8`
//! (WP28, `STUDIES/funnel_defaults_v1`), and that it adds the `stanreg` arm:
//! the sampler's default warmup (`WarmupConfig::new(0.8)` with the WP24
//! exhaustion rule) plus `DiagonalMetricRegularization::Stan` (WP27,
//! `STUDIES/step_collapse_v1`). The `paper` and `stan` arms are kept in the
//! binary but not run in v4. The refinement-level histogram of the retained
//! transitions is recorded per chain as in v3 (`retained_refinement_level_histogram`).
//!
//! Usage: `posteriordb-bench-v4 <model.so> <data.json> <da|stanreg|paper|stan> <seed> <out.json> [threads]`
//!
...
//! `h0 = 0.5`, depth 10, eight refinement levels, `delta = 1`; adaptive
...
//! `stanreg` = `Adaptation::Custom(WarmupConfig::new(0.8)
...
//! `stan` = `Adaptation::Custom(WarmupConfig::stan_style(0.8))`.
...
use owalnuts::sampler::{
    Adaptation, DEFAULT_WARMUP_EXHAUSTION, Init, Limits, Metric, PaperAdaptationConfig, Sampler,
    Target, Tuning, WarmupConfig, uniform_starts,
};
use owalnuts::walnutpie::{
    ALGORITHM_REVISION, DiagonalMetricRegularization, PAPER_ADAPTATION_REVISION,
};
...
/// The sampler's default warmup as a `WarmupConfig` plus Stan's diagonal
/// metric regularisation (the `reg` arm of `STUDIES/step_collapse_v1`).
fn stanreg_config() -> Result<WarmupConfig, Box<dyn Error>> {
    Ok(WarmupConfig::new(TARGET_ACCEPT)?
        .with_warmup_exhaustion_rule(DEFAULT_WARMUP_EXHAUSTION)
        .with_metric_regularization(DiagonalMetricRegularization::Stan))
}
...
fn adaptation(arm: &str) -> Result<Adaptation, Box<dyn Error>> {
    Ok(match arm {
        ...
        "stanreg" => Adaptation::Custom(stanreg_config()?),
        "paper" => Adaptation::Paper(PaperAdaptationConfig::default()),
        "stan" => Adaptation::Custom(WarmupConfig::stan_style(TARGET_ACCEPT)?),
        other => return Err(format!("unknown arm {other:?} (expected da|stanreg|paper|stan)").into()),
    })
}
    ...
    match arm {
        ...
        "stan" => {
            ...
            json!({"mode": "stan_style", "target_accept": w.target_acceptance(),
                   ...
                   "warmup_exhaustion_rule": format!("{:?}", w.warmup_exhaustion_rule()),
                   "mass_adaptation": true})
        }
        "stanreg" => {
            let w = stanreg_config().expect("valid config");
            json!({"mode": "dual_averaging_stan_regularization", "target_accept": w.target_acceptance(),
                   "dual_averaging_acceptance": format!("{:?}", w.dual_averaging_acceptance()),
                   "initial_step_search": w.initial_step_search().is_some(),
                   "metric_regularization": format!("{:?}", w.metric_regularization()),
                   "stan_restart_reference": w.stan_restart_reference(),
        ...
        _ => json!({"mode": "dual_averaging", "target_accept": TARGET_ACCEPT,
                    "metric_regularization": "TowardUnit (default)",
                    "warmup_exhaustion_rule": format!("{DEFAULT_WARMUP_EXHAUSTION:?}"),
                    "mass_adaptation": true}),
    }
}
...
fn run(
    ...
    let starts = match uniform_starts(&target, CHAINS, seed, radius, max_attempts) {
        ...
        Err(e) => {
            let payload = json!({
                "schema": "posteriordb-bench-v4-cell",
...
fn main() {
    ...
    let result: Result<(), Box<dyn Error>> = match args.as_slice() {
        ...
        _ => Err("usage: <model.so> <data.json> <da|stanreg|paper|stan> <seed> <out.json> [threads]".into()),
    };
