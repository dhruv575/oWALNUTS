#!/usr/bin/env python3
"""posteriordb benchmark v3 driver (the v2 protocol on the WP24 warmup default, fresh seeds, four arms).
...
HARNESS = HERE / "target" / "release" / "posteriordb-bench-v3.exe"
ARMS = ["owalnuts-da", "owalnuts-stan-style", "cmdstan", "nutpie"]
OWALNUTS_MODE = {"owalnuts-da": "da", "owalnuts-stan-style": "stan"}
V2_SUMMARY = HERE.parent / "posteriordb_bench_v2" / "artifacts" / "summary.json"
V2_ARMS = {"owalnuts-da": "owalnuts-da", "owalnuts-stan-style": "owalnuts-stan-style", "cmdstan": "cmdstan", "nutpie": "nutpie"}
...
def metrics(model: str, arm: str, seed: int, draws: np.ndarray, names: list[str], ref: dict,
    ...
    return {
        "schema": "posteriordb-bench-v3-cell-metrics",
...
def failure(model: str, arm: str, seed: int, status: str, message: str, **extra) -> dict:
    return {
        "schema": "posteriordb-bench-v3-cell-metrics", "model": model, "arm": arm, "seed": seed,
...
def analyze() -> None:
    ...
    for arm in ("cmdstan", "nutpie"):
        for ours in ("owalnuts-da", "owalnuts-stan-style"):
    ...
    # v2 -> v3 comparison (seed medians; identical protocol; the v2 kernel default versus the WP24 default).
    v2 = json.loads(V2_SUMMARY.read_text(encoding="utf-8"))["per_model"] if V2_SUMMARY.exists() else {}
    versus_v2 = {}
    for model in models:
        row = {}
        for arm, v2arm in V2_ARMS.items():
            a, b = per_model[model][arm], v2.get(model, {}).get(v2arm, {})

            def r(k, a=a, b=b):
                x = a[k]["median"]
                y = (b.get(k) or {}).get("median")
                return (x / y) if (x and y) else None
            row[arm] = {"v2_passed": b.get("n_passed"), "v3_passed": a["n_passed"], "v2_ok": b.get("n_ok"), "v3_ok": a["n_ok"],
                        ...
                        "v2_bulk_ess_per_gradient": (b.get("min_bulk_ess_per_gradient") or {}).get("median"),
                        "v3_bulk_ess_per_gradient": a["min_bulk_ess_per_gradient"]["median"]}
        versus_v2[model] = row
    versus_v2_overall = {arm: {
        "models": [m for m in models if versus_v2[m][arm]["r_bulk_ess_per_gradient"]],
        "geomean_r_bulk_ess_per_gradient": geomean([versus_v2[m][arm]["r_bulk_ess_per_gradient"] for m in models]),
        "geomean_r_bulk_ess_per_second": geomean([versus_v2[m][arm]["r_bulk_ess_per_second"] for m in models]),
        "v2_cells_passed": sum((versus_v2[m][arm]["v2_passed"] or 0) for m in models),
        "v3_cells_passed": sum(versus_v2[m][arm]["v3_passed"] for m in models),
    } for arm in V2_ARMS}
    ...
    lv = "hudson_lynx_hare-lotka_volterra"
    excl = [m for m in hc["models"] if m != lv]
    geo_excl = geomean([per_model[m]["owalnuts-da"]["min_bulk_ess_per_gradient"]["median"] / per_model[m]["cmdstan"]["min_bulk_ess_per_gradient"]["median"] for m in excl])
    regressed = {m: versus_v2[m]["owalnuts-da"]["r_bulk_ess_per_gradient"] for m in models
                 if versus_v2[m]["owalnuts-da"]["r_bulk_ess_per_gradient"] is not None and versus_v2[m]["owalnuts-da"]["r_bulk_ess_per_gradient"] < 0.8}
    arma_da = per_model["arma-arma11"]["owalnuts-da"]["n_passed"]
    n_pass = sum(1 for c in da_cells if c.get("passed"))
    predictions = {
        "P1_da_gate_passes_ge_33_of_51": {"value": n_pass, "held": n_pass >= 33},
        "P2_da_arma11_gate_passes_ge_2_of_3": {"value": arma_da, "held": arma_da >= 2},
        "P3_da_geomean_bulk_ess_per_gradient_vs_cmdstan_ge_0.40_all_models": {"value": hc["geomean_bulk_ess_per_gradient"], "models": len(hc["models"]), "held": bool(hc["geomean_bulk_ess_per_gradient"] >= 0.40)},
        "P3b_da_geomean_bulk_ess_per_gradient_vs_cmdstan_ge_0.45_excluding_lotka_volterra": {"value": geo_excl, "models": len(excl), "held": bool(geo_excl >= 0.45)},
        "P4_no_model_da_bulk_ess_per_gradient_below_0.8x_v2": {"value": regressed, "held": not regressed},
    ...
    summary = {
        "schema": "posteriordb-bench-v3-summary",
        "versus_v2": versus_v2,
        "versus_v2_overall": versus_v2_overall,
    ...
    lines = ["# posteriordb benchmark v3 — results", "",
    ...
    lines += ["", "## v3 versus v2 (seed medians; ratio v3 / v2; same protocol and settings, seeds 78101-78103 -> 79101-79103; the only sampler change is the WP24 warmup exhaustion rule in the DA arm)", "",
              "| model | arm | v2 gates | v3 gates | v2 ESS/grad x1e3 | v3 ESS/grad x1e3 | ESS/grad v3/v2 | ESS/s v3/v2 | wall v3/v2 | grads v3/v2 |", "|---|---|---|---|---:|---:|---:|---:|---:|---:|"]
    for model in models:
        for arm in V2_ARMS:
            v = versus_v2[model][arm]
            lines.append(f"| {model} | {arm} | {v['v2_passed']}/3 | {v['v3_passed']}/3 | {f((v['v2_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                         f"{f((v['v3_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | {f(v['r_bulk_ess_per_gradient'], '.2f')} | {f(v['r_bulk_ess_per_second'], '.2f')} | {f(v['r_wall'], '.2f')} | {f(v['r_gradients'], '.2f')} |")
    lines += ["", "| arm | v2 cells passed | v3 cells passed | geomean ESS/grad v3/v2 | geomean ESS/s v3/v2 |", "|---|---:|---:|---:|---:|"]
    for arm, o in versus_v2_overall.items():
        lines.append(f"| {arm} | {o['v2_cells_passed']} | {o['v3_cells_passed']} | {f(o['geomean_r_bulk_ess_per_gradient'], '.2f')} | {f(o['geomean_r_bulk_ess_per_second'], '.2f')} |")
