//! oWALNUTS arms of the posteriordb benchmark v3: one (model, arm, seed) cell.
//!
//! Identical to the v2 harness except that it is built against the current
//! `src/`, whose `sampler` defaults apply `DEFAULT_WARMUP_EXHAUSTION =
//! ExhaustionRule::AcceptUnlessDivergent` to warmup transitions (WP24); the
//! `paper` arm is kept in the binary but not run in v3.
//!
//! Usage: `posteriordb-bench-v3 <model.so> <data.json> <da|paper|stan> <seed> <out.json> [threads]`
//!
...
fn run(
    ...
    let starts = match uniform_starts(&target, CHAINS, seed, radius, max_attempts) {
        ...
        Err(e) => {
            let payload = json!({
                "schema": "posteriordb-bench-v3-cell",
