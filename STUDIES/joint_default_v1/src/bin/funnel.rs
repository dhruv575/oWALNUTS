//! Neal's 10-D funnel tail-mass check under one of the four arms, at the
//! paper tuning or the sampler defaults.
//!
//! Usage: `funnel <paper|defaults> <da|rhosum|stanreg|joint> <seed> <out.json>`
//!
//! * `paper`: the `examples/funnel_kernel_options.rs` protocol (Appendix C
//!   warmup, identity metric, `h = 0.1`, `delta = 1`, eight refinement
//!   levels, depth 10). The identity metric is not adapted, so the
//!   regularisation is inert here: `stanreg` = `da` and `joint` = `rhosum`
//!   by construction (both are run and reported anyway).
//! * `defaults`: `Tuning::default()` (eight levels since WP28), adapted
//!   diagonal metric, dual averaging at 0.8 with the WP24 warmup rule, plus
//!   the arm's U-turn rule and regularisation.
//!
...
fn main() {
    ...
    let result: Result<(), Box<dyn Error>> = match args.as_slice() {
        ...
        _ => Err("usage: <paper|defaults> <da|rhosum|stanreg|joint> <seed> <out.json>".into()),
    };
