//! Neal's 10-D funnel tail-mass row of the posteriordb benchmark v5: the
//! sampler defaults after the post-WP31 default change (`Tuning::default()`
//! with the momentum-sum U-turn rule and eight refinement levels,
//! `Metric::diagonal()` with Stan's regularisation, dual averaging at 0.8
//! with the WP24 warmup rule, cached initial evaluation), nothing overridden.
//!
...
fn run(seed: u64, out: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let retained_calls: usize = posterior
        ...
        .sum();
    let payload = json!({
        "schema": "posteriordb-bench-v5-funnel",
        "tuning": "defaults",
        "arm": "owalnuts-da",
