//! oWALNUTS arm of the posteriordb benchmark v5: one (model, arm, seed) cell.
//!
//! The v3 harness built against the current `src/`, whose `sampler`
//! defaults are, since the post-WP31 default change, `Tuning::default()` with
//! `UTurnRule::MomentumSum` and `Adaptation::DualAveraging` with
//! `DiagonalMetricRegularization::Stan` (plus the WP24 warmup exhaustion
//! rule and the WP28 eight refinement levels). The only arm run in v5 is
//! `da` = the shipped defaults; `paper` and `stan` are kept for parity with
//! the v3 binary and are not run. The JSON records the kernel options and
//! the regularisation so that the cell is self-describing.
//!
//! Usage: `posteriordb-bench-v5 <model.so> <data.json> <da|paper|stan> <seed> <out.json> [threads]`
//!
...
use owalnuts::sampler::{
    Adaptation, DEFAULT_METRIC_REGULARIZATION, DEFAULT_U_TURN_RULE, DEFAULT_WARMUP_EXHAUSTION, Init,
    Limits, Metric, PaperAdaptationConfig, Sampler, Target, Tuning, WarmupConfig, uniform_starts,
};
...
fn run(
    ...
    let starts = match uniform_starts(&target, CHAINS, seed, radius, max_attempts) {
        ...
        Err(e) => {
            let payload = json!({
                "schema": "posteriordb-bench-v5-cell",
