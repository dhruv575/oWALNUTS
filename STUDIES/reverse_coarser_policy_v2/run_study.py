#!/usr/bin/env python3
"""Second reverse-coarser policy study driver (WP39B): the posteriordb v5 protocol on four
oWALNUTS arms: `stop` (shipped), `beyond-adapt` (`ZeroWeightBeyondAdaptSelected` under the shipped
adaptation), and the fixed-step pair `stop-fixed` / `beyond-fixed` at the `stop` cell's adapted step
(same model, same seed), which isolates orbit truncation from the step. CmdStan cited from v5.
    ...
    python run_study.py checks                     # funnel tail mass at the defaults, adapted-step arms, three seeds
...
BASELINE = "stop"
CANDIDATE = "beyond-adapt"
FIXED_BASE = "stop-fixed"
FIXED_CAND = "beyond-fixed"
...
def metrics(model: str, arm: str, seed: int, draws: np.ndarray, names: list[str], ref: dict,
    ...
    return {
        "schema": "reverse-coarser-policy-v2-cell-metrics",
...
def failure(model: str, arm: str, seed: int, status: str, message: str, **extra) -> dict:
    return {
        "schema": "reverse-coarser-policy-v2-cell-metrics", "model": model, "arm": arm, "seed": seed,
...
def run_owalnuts(model: str, arm: str, seed: int, ref: dict) -> dict:
    ...
    if arm.endswith("-fixed"):
        base = cell_path(model, BASELINE, seed)
        if not base.exists():
            return failure(model, arm, seed, "missing_reference_step", f"no {BASELINE} cell for seed {seed}")
        base_cell = json.loads(base.read_text(encoding="utf-8"))
        if base_cell.get("status") != "ok":
            return failure(model, arm, seed, "missing_reference_step", f"{BASELINE} cell for seed {seed} is {base_cell.get('status')}")
        fixed_step = float(base_cell["median_step"])
    raw = DRAWS / f"{short(model)}-{arm}-{seed}.raw.json"
    DRAWS.mkdir(parents=True, exist_ok=True)
    if raw.exists():
        raw.unlink()
    cmd = [str(HARNESS), str(so), str(data), arm, str(seed), str(raw), "4"] + ([repr(fixed_step)] if fixed_step is not None else [])
    try:
...
def run_checks(arms: list[str]) -> None:
    ...
    for arm in [a for a in arms if not a.endswith("-fixed")]:
        for seed in SEEDS:
...
KEYS = ["min_bulk_ess_per_gradient", "min_bulk_ess_per_gradient_sampling", "min_tail_ess_per_gradient",
        ...
        "gradients_total", "gradients_sampling", "wall_seconds", "median_step", "fixed_step", "mean_depth",
        ...
        "retained_forward_refinement_attempts", "max_abs_z", "warmup_divergences"]
...
def analyze() -> None:
    ...
    for model in models:
        pm = per_model[model]
        s = pm[BASELINE]
        r = {"vs_stop": {}, "step_vs_stop": {}, "vs_cmdstan": {}}
        for arm in ARMS:
            r["vs_stop"][arm] = ratio(pm[arm]["min_bulk_ess_per_gradient"], s["min_bulk_ess_per_gradient"])
            r["step_vs_stop"][arm] = ratio(pm[arm]["median_step"], s["median_step"])
            r["vs_cmdstan"][arm] = ratio(pm[arm]["min_bulk_ess_per_gradient"], cmdstan.get(model, {}).get("min_bulk_ess_per_gradient"))
        r["ratio"] = r["vs_stop"][CANDIDATE]
        r["ratio_sampling"] = ratio(pm[CANDIDATE]["min_bulk_ess_per_gradient_sampling"], s["min_bulk_ess_per_gradient_sampling"])
        r["ratio_per_second"] = ratio(pm[CANDIDATE]["min_bulk_ess_per_second"], s["min_bulk_ess_per_second"])
        r["fixed_ratio"] = ratio(pm[FIXED_CAND]["min_bulk_ess_per_gradient"], pm[FIXED_BASE]["min_bulk_ess_per_gradient"])
        r["fixed_ratio_sampling"] = ratio(pm[FIXED_CAND]["min_bulk_ess_per_gradient_sampling"], pm[FIXED_BASE]["min_bulk_ess_per_gradient_sampling"])
        r["stop_fixed_vs_stop"] = r["vs_stop"][FIXED_BASE]
        ratios[model] = r
    all_ratios = [ratios[m]["ratio"] for m in models]
    complete = all(r is not None for r in all_ratios)
    fixed_complete = all(ratios[m]["fixed_ratio"] is not None for m in models)
    ...
    decision = {
        ...
        "geomean_step_ratio": geomean([ratios[m]["step_vs_stop"][CANDIDATE] for m in models]),
        "max_abs_log_step_ratio": max((abs(math.log(ratios[m]["step_vs_stop"][CANDIDATE])) for m in models
                                       if ratios[m]["step_vs_stop"][CANDIDATE]), default=None),
        ...
        "funnel_candidate_all_seeds_abs_z_le_2": funnel[CANDIDATE]["all_seeds_abs_z_le_2"],
    ...
    decision["C4_funnel"] = bool(decision["funnel_candidate_all_seeds_abs_z_le_2"])
    ...
    mechanism = {
        ...
        "fixed_geomean_all": geomean([ratios[m]["fixed_ratio"] for m in models]),
        "fixed_geomean_targets": geomean([ratios[m]["fixed_ratio"] for m in TARGET_MODELS]),
        "fixed_geomean_controls": geomean([ratios[m]["fixed_ratio"] for m in control]),
        "fixed_geomean_sampling_all": geomean([ratios[m]["fixed_ratio_sampling"] for m in models]),
        "fixed_min_ratio": min((ratios[m]["fixed_ratio"] for m in models if ratios[m]["fixed_ratio"] is not None), default=None),
        "stop_fixed_vs_stop_geomean": geomean([ratios[m]["stop_fixed_vs_stop"] for m in models]),
        "gates_fixed": {FIXED_BASE: gates[FIXED_BASE], FIXED_CAND: gates[FIXED_CAND]},
    }
    mechanism["M1_targets"] = bool(fixed_complete and mechanism["fixed_geomean_targets"] >= rule["mechanism_targets_min"])
    mechanism["M2_controls_within"] = bool(fixed_complete and abs(math.log(mechanism["fixed_geomean_controls"])) <= math.log(rule["mechanism_controls_band"]))
    mechanism["truncation_hypothesis_supported"] = mechanism["M1_targets"] and mechanism["M2_controls_within"]

    summary = {
        "schema": "reverse-coarser-policy-v2-summary",
        ...
        "per_model": per_model, "ratios": ratios, "cmdstan_v5": cmdstan, "decision": decision,
        "mechanism": mechanism, "funnel": funnel,
    }
    (ART / "summary.json").write_text(json.dumps(summary, indent=1, sort_keys=True, default=float) + "\n", encoding="utf-8")
    write_table(models, per_model, ratios, decision, mechanism, funnel)
    log(f"decision: {json.dumps({k: v for k, v in decision.items() if k.startswith('C') or k in ('flip', 'geomean_all', 'geomean_targets', 'geomean_step_ratio', 'min_ratio', 'argmin_ratio', 'gates')})}")
    log(f"mechanism: {json.dumps(mechanism)}")
...
def write_table(models, per_model, ratios, decision, mechanism, funnel) -> None:
    rule = PROTOCOL["decision_rule"]
    L = ["# reverse_coarser_policy_v2 — results", "",
         ...
         "ESS >= 400, zero divergences (of 3 per arm). `stop` = shipped defaults; `beyond-adapt` = `ZeroWeightBeyondAdaptSelected` "
         "under the shipped adaptation; `stop-fixed` / `beyond-fixed` = the two policies at the `stop` cell's adapted step "
         "(same seed), mass adapted, step fixed. `rc stop` = reverse-coarser stops per retained transition (`stop`); `cont` = "
         "continued leaves per retained transition; `zero-w` = zero-weight leaves / built leaves.", "",
    ...
    for m in models:
        ...
        L.append(f"| {m}{tag} | {f((s['min_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                 ...
                 f"{f(r['ratio_sampling'], '.3f')} | {s['gates_passed']}/{b['gates_passed']} | "
                 f"{f(r['step_vs_stop'][CANDIDATE], '.3f')} | {f(s['leaves_per_transition'], '.1f')} → {f(b['leaves_per_transition'], '.1f')} | "
    ...
    L += ["", "## Fixed step: `beyond-fixed` vs `stop-fixed` (the mechanism comparison)", "",
          ...
          "|---|---:|---:|---:|---:|---:|---|---:|---|---:|---:|---:|---|"]
    for m in models:
        s, b, r = per_model[m][FIXED_BASE], per_model[m][FIXED_CAND], ratios[m]
        tag = " **(target)**" if m in TARGET_MODELS else ""
        L.append(f"| {m}{tag} | {f(s['fixed_step'], '.4g')} | {f((s['min_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | "
                 f"{f((b['min_bulk_ess_per_gradient'] or float('nan')) * 1e3, '.3f')} | {f(r['fixed_ratio'], '.3f')} | "
                 f"{f(r['fixed_ratio_sampling'], '.3f')} | {s['gates_passed']}/{b['gates_passed']} | {f(r['stop_fixed_vs_stop'], '.3f')} | "
                 f"{f(s['leaves_per_transition'], '.1f')} → {f(b['leaves_per_transition'], '.1f')} | "
                 f"{f(s['reverse_coarser_stop_fraction'], '.3f')} | {f(b['reverse_coarser_continuation_fraction'], '.3f')} | "
                 f"{f(b['zero_weight_leaf_fraction'], '.3f')} | {f(s['max_depth_stop_fraction'], '.3f')} → {f(b['max_depth_stop_fraction'], '.3f')} |")
    d, mch = decision, mechanism
    L += ["", "## Decision (protocol.json `decision_rule`; `beyond-adapt` vs `stop`)", "",
          "| statistic | value |", "|---|---:|",
          f"| geomean ratio, 17 models | {f(d['geomean_all'], '.3f')} |",
          ...
          f"| geomean adapted-step ratio adapt/stop (max \\|log\\|) | {f(d['geomean_step_ratio'], '.3f')} ({f(d['max_abs_log_step_ratio'], '.3f')}) |",
          f"| worst model | {f(d['min_ratio'], '.3f')} ({d['argmin_ratio']}) |",
          f"| gates passed (of 51) stop / beyond-adapt | {d['gates'][BASELINE]} / {d['gates'][CANDIDATE]} |",
          f"| vs CmdStan (healthy) stop / beyond-adapt | {f(d['vs_cmdstan_healthy'][BASELINE], '.3f')} / {f(d['vs_cmdstan_healthy'][CANDIDATE], '.3f')} |",
          f"| C1 geomean >= {rule['geomean_min']} | {d['C1_geomean']} |",
          f"| C2 no model < {rule['per_model_min']} | {d['C2_no_model_below']} |",
          f"| C3 gates >= stop | {d['C3_gates']} |",
          f"| C4 funnel \\|z\\| <= 2 every seed (beyond-adapt) | {d['C4_funnel']} |",
          f"| C5 targets geomean >= {rule['targets_geomean_min']} | {d['C5_targets']} |",
          f"| **flip the default** | **{d['flip']}** |",
          "", "## Mechanism (`beyond-fixed` vs `stop-fixed`; registered predictions M1, M2)", "",
          "| statistic | value |", "|---|---:|",
          f"| geomean fixed ratio, 17 models | {f(mch['fixed_geomean_all'], '.3f')} |",
          f"| geomean fixed ratio, 4 target models | {f(mch['fixed_geomean_targets'], '.3f')} |",
          f"| geomean fixed ratio, 13 controls | {f(mch['fixed_geomean_controls'], '.3f')} |",
          f"| geomean fixed ratio on sampling gradients | {f(mch['fixed_geomean_sampling_all'], '.3f')} |",
          f"| worst fixed ratio | {f(mch['fixed_min_ratio'], '.3f')} |",
          f"| stop-fixed vs stop (geomean; fixing the step costs nothing if ~1) | {f(mch['stop_fixed_vs_stop_geomean'], '.3f')} |",
          f"| gates stop-fixed / beyond-fixed | {mch['gates_fixed'][FIXED_BASE]} / {mch['gates_fixed'][FIXED_CAND]} |",
          f"| M1 targets geomean >= {rule['mechanism_targets_min']} | {mch['M1_targets']} |",
          f"| M2 controls within {rule['mechanism_controls_band']}x | {mch['M2_controls_within']} |",
          f"| **truncation hypothesis supported** | **{mch['truncation_hypothesis_supported']}** |",
          ...
          "|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---:|---:|"]
    for arm, entry in funnel.items():
        for seed, p in entry["per_seed"].items():
