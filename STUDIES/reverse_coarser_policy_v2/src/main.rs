//! oWALNUTS arms of the second reverse-coarser policy study (WP39B): one (model, arm, seed) cell.
//!
//! The posteriordb v5 harness (`STUDIES/posteriordb_bench_v5/src/main.rs`)
//! with the arm selecting `Tuning::reverse_coarser_policy` and whether the
//! step is adapted or fixed (see `arms.rs`),
...
//!
//! Usage: `posteriordb-cell <model.so> <data.json> <arm> <seed> <out.json> [threads] [fixed_step]`
//!
...
fn run(
    ...
    let tuning = arm.tuning(fixed_step)?;
    ...
    let starts = match uniform_starts(&target, CHAINS, seed, radius, max_attempts) {
        ...
        Err(e) => {
            let payload = json!({
                "schema": "reverse-coarser-policy-v2-cell",
    ...
    let payload = json!({
        ...
        "warmup_config": arm.json(fixed_step),
...
fn main() {
    ...
    let result: Result<(), Box<dyn Error>> = match args.as_slice() {
        [model, data, arm, seed, out, rest @ ..] => {
            ...
            let fixed = rest.get(1).map(|h| h.parse::<f64>());
            match (seed.parse::<u64>(), threads, fixed) {
                (Ok(s), Ok(t), None) => {
                ...
                _ => Err("seed and threads must be integers, fixed_step a number".into()),
            }
        }
        _ => Err("usage: <model.so> <data.json> <arm> <seed> <out.json> [threads] [fixed_step]".into()),
    };
