"""Post-processing only (v2): gates, stability, landing, and efficiency vs the F9 control.
...
def analyze(name):
    ...
    gates["delta_spread"] = out["final_delta_spread_ratio"] <= G["final_delta_spread_max"]
    gates["step_spread"] = out["final_step_spread_ratio"] <= G["final_step_spread_max"]
    out["gates"] = gates
    out["transitions_without_statistic"] = int(sum(
        u["transitions_without_statistic"] for c in a["chains"] for u in c["paper_adaptation_updates"]))
    out["dual_averaging_restarts"] = int(sum(
        u["dual_averaging_restarted"] for c in a["chains"] for u in c["paper_adaptation_updates"]))
...
ARMS = ("A2-C", "A2-S", "A2-R", "A2-SR", "AD-C", "AD-S", "AD-R", "AD-SR")


def main():
    arms = [r for r in (analyze(n) for n in ARMS) if r]
    ctrl = control()
    for o in arms:
        if ctrl and o["mode"] == "paper":
            ...
            o["gates"]["bulk_ess_per_call_vs_F9"] = o["bulk_ess_per_call_ratio_vs_F9"] >= G["bulk_ess_per_call_ratio_min"]
            o["gates"]["tail_ess_per_call_vs_F9"] = o["tail_ess_per_call_ratio_vs_F9"] >= G["tail_ess_per_call_ratio_min"]
            o["all_gates_pass"] = all(o["gates"].values())
            o["unbiased_and_healthy"] = all(o["gates"][k] for k in (
                "tail_mass_minus5", "tail_mass_minus6", "omega_variance", "health",
                "rhat_omega", "rhat_x1", "bulk_ess_omega", "tail_ess_omega", "bulk_ess_x1", "tail_ess_x1"))
            o["stable"] = o["gates"]["delta_spread"] and o["gates"]["step_spread"]
            o["efficient"] = o["gates"]["bulk_ess_per_call_vs_F9"] and o["gates"]["tail_ess_per_call_vs_F9"]
    summary = {"schema": "owalnuts-paper-funnel-adaptive-summary/v2", "executed_utc_date": "2026-08-31",
    ...
    print("| arm | Δ | final δ per chain | final h per chain | R-hat ω/x1 | bulk/tail ESS ω | var ω | "
          "P(ω<-5) | P(ω<-6) | q1% | div/inval/exhaust/cap | retained calls | bulk/tail ESS/call ×F9 | "
          "h spread | δ spread | unbiased+healthy / stable / efficient |")
    print("|" + "---|" * 16)
    for o in arms:
        f = o["functionals"]
        h = o["health"]
        ...
        print(f"| {o['arm']} | {d} | {', '.join(f'{x:.3f}' for x in o['final_delta_per_chain'])} | "
              ...
              f"{o.get('tail_ess_per_call_ratio_vs_F9', float('nan')):.2f} | "
              f"{o['final_step_spread_ratio']:.2f} | {o['final_delta_spread_ratio']:.3f} | "
              f"{'Y' if o.get('unbiased_and_healthy') else 'N'} / {'Y' if o.get('stable') else 'N'} / "
              f"{'Y' if o.get('efficient') else 'N'} |")
        if o["mode"] == "paper":
            print(f"  updates chain 0: " + "; ".join(
                ...
                f"h {u['step_before']:.3f}->{u['step_after']:.3f} stat={u['step_statistic'] and round(u['step_statistic'], 3)} "
                f"restart={u['dual_averaging_restarted']} nostat={u['transitions_without_statistic']} [{u['outcome']}]"
