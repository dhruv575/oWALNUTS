...
//! Paper funnel adaptive-warmup runner v2 (WP9): h-rule stabilisation arms. Reads `protocol.json`, runs one
...
use owalnuts::walnutpie::{
    DiagonalMass, KernelTuning, PaperAdaptationConfig, PaperAdaptationOutcome,
    PaperRestartPolicy, PaperStepStatistic, RunConfig, StopReason, Target, TargetError, TargetEvaluationAdmissionLimit, TargetEvaluationBudget,
    ...
    let paper = match a["mode"].as_str() {
        ...
        Some("paper") => {
            ...
            Some(Paper {
                ...
                step_statistic: p["step_statistic"]
                    .as_str()
                    .ok_or("paper.step_statistic missing")?
                    .to_string(),
                restart_policy: p["restart_policy"]
                    .as_str()
                    .ok_or("paper.restart_policy missing")?
                    .to_string(),
            })
        }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if mode == "--preflight" {
        ...
        let result = json!({
            "schema": "owalnuts-paper-funnel-adaptive-preflight/v2",
        ...
        let updates: Vec<Value> = chain
            ...
            .map(|u| {
                json!({
                    ...
                    "step_statistic": u.step_statistic(),
    ...
    let report = json!({
        "schema": "owalnuts-paper-funnel-adaptive-arm/v2",
        "arm": arm.name,
