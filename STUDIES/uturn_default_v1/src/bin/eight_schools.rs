...
//! timing boundary copied verbatim) under a selectable U-turn rule.
//!
//! Usage: `eight-schools <endpoints|rhosum|cross> <seed> <reps> <out.json>`
//!
...
const SEEDS: [u64; 3] = [80_101, 80_102, 80_103];
...
fn config(seed: u64, u_turn: UTurnRule) -> Result<RunConfig, Box<dyn Error>> {
...
fn run_seed(rule: &str, seed: u64, reps: usize, out: &Path) -> Result<(), Box<dyn Error>> {
    if out.exists() {
    ...
    let chains = sampled
        ...
        .collect::<Vec<_>>();
    let payload = json!({
        "schema": "uturn-default-v1-eight-schools-cell",
        "u_turn": format!("{u_turn:?}"),
        "seed": seed,
        "starts": starts(),
...
fn main() {
    ...
    let result: Result<(), Box<dyn Error>> = match args.as_slice() {
        [rule, seed, reps, out] => match (seed.parse::<u64>(), reps.parse::<usize>()) {
            (Ok(s), Ok(r)) => run_seed(rule, s, r, Path::new(out)),
            _ => Err("seed and reps must be integers".into()),
        },
        _ => Err("usage: <endpoints|rhosum|cross> <seed> <reps> <out.json>".into()),
    };
