...
`oracle/walnutpie/f5bba365_invalid_leaves` adds 4,000 funnel macro leaves
against a hard wall where the upstream `NoExceptLogpGrad` wrapper maps a
throwing evaluation to `logp = -inf`, `grad = 0`; they pin the `v10` rule that
a recoverable failure refines like any over-tolerance micro-step instead of
